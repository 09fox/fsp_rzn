...
#if (1 == BSP_LP64_SUPPORT)
    return bsp_prv_conversion_uint_to_float(R_TFU->SCDT1_b.SCDT1);
#else
    return R_TFU->SCDT1;
#endif
}
...
#if (1 == BSP_LP64_SUPPORT)
    return bsp_prv_conversion_uint_to_float(R_TFU->SCDT0_b.SCDT0);
#else
    return R_TFU->SCDT0;
#endif
}
...
BSP_TFU_INLINE void __sincosf (float angle, float * sin, float * cos)
{
    ...
    /* Read sin from R_TFU->SCDT1 */
    *sin = bsp_prv_conversion_uint_to_float(R_TFU->SCDT1_b.SCDT1);

    /* Read cos from R_TFU->SCDT0 */
    *cos = bsp_prv_conversion_uint_to_float(R_TFU->SCDT0_b.SCDT0);
#else
...
#if (1 == BSP_LP64_SUPPORT)
    return bsp_prv_conversion_uint_to_float(R_TFU->ATDT1_b.ATDT1);
#else
    return R_TFU->ATDT1;
#endif
}
...
#if (1 == BSP_LP64_SUPPORT)
    return bsp_prv_conversion_uint_to_float(R_TFU->ATDT0_b.ATDT0) * R_TFU_HYPOT_SCALING_FACTOR;
#else
...
BSP_TFU_INLINE void __atan2hypotf (float y_cord, float x_cord, float * atan2, float * hypot)
{
    ...
    /* Read arctan(y/x) from R_TFU->ATDT1 */
    *atan2 = bsp_prv_conversion_uint_to_float(R_TFU->ATDT1_b.ATDT1);

    /* Read sqrt (x_cord2 + y_cord2) from R_TFU->ATDT0 */
    *hypot = bsp_prv_conversion_uint_to_float(R_TFU->ATDT0_b.ATDT0) * R_TFU_HYPOT_SCALING_FACTOR;
#else
