...
#define BSP_PRV_PMSEL_BASE_ADDRESS       (0x81280010)
#define BSP_PRV_PMSEL_PMSEL0MON_SHIFT    (9U)
#define BSP_PRV_PMSEL_PMSEL2MON_SHIFT    (11U)
#define BSP_PRV_PMSEL_PMSEL3MON_SHIFT    (12U)
#define BSP_PRV_PMSEL_PMSELMON_MASK      (0x01)
...
/* Calculate the value to write to PLL3_VCO_CTR */
 ...
 #define BSP_PRV_STARTUP_PLL3_VCO_CTR0      ((BSP_CFG_PLL3P << 16) | \
                                             (((BSP_CFG_PLL3M_PLL3K >> 16) & 0x3FF) << 0))

 #define BSP_PRV_STARTUP_PLL3_VCO_CTR1      (((BSP_CFG_PLL3M_PLL3K & 0xFFFF) << 16) | \
                                             (BSP_CFG_PLL3S << 0))
#endif
...
void bsp_clock_init (void)
{
    ...
    if (BSP_CLOCKS_PLL0_NORMAL == BSP_CFG_PLL0)
    {
        if (BSP_CLOCKS_PLL0_STANDBY == R_SYSC_S->PLL0EN_b.PLL0EN)
        {
            ...
            FSP_HARDWARE_REGISTER_WAIT(((*((uint32_t *) BSP_PRV_PMSEL_BASE_ADDRESS) >> BSP_PRV_PMSEL_PMSEL0MON_SHIFT) &
                                        BSP_PRV_PMSEL_PMSELMON_MASK),
                                       1U);
        }
    }
#endif
    ...
    if (BSP_CLOCKS_PLL2_NORMAL == BSP_CFG_PLL2)
    {
        if (BSP_CLOCKS_PLL2_STANDBY == R_SYSC_S->PLL2EN_b.PLL2EN)
        {
            ...
            /* Confirm clock source change of PLL clock domain. */
            FSP_HARDWARE_REGISTER_WAIT(((*((uint32_t *) BSP_PRV_PMSEL_BASE_ADDRESS) >> BSP_PRV_PMSEL_PMSEL2MON_SHIFT) &
                                        BSP_PRV_PMSEL_PMSELMON_MASK),
                                       1U);
        }
    }
    ...
    if (BSP_CLOCKS_PLL3_NORMAL == BSP_CFG_PLL3)
    {
        if (BSP_CLOCKS_PLL3_STANDBY == R_SYSC_S->PLL3EN_b.PLL3EN)
        {
            ...
            /* Confirm clock source change of PLL clock domain. */
            FSP_HARDWARE_REGISTER_WAIT(((*((uint32_t *) BSP_PRV_PMSEL_BASE_ADDRESS) >> BSP_PRV_PMSEL_PMSEL3MON_SHIFT) &
                                        BSP_PRV_PMSEL_PMSELMON_MASK),
                                       1U);
        }
    }
